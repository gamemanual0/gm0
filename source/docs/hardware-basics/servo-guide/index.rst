...
position. Typically servos have very  limited range of rotation
(180 degrees is common).

**Servos are NOT replacements for DC motors, and should not be used
...
(PWM signal at 50 hz). REV Expansion hub provides 6 servo connectors, so you
...
use servos used in FTC are the REV Smart Servo and  goBILDA Dual mode Servo
(25-2), but you should  check out other servos as well. To learn more,
please read the  :doc:`Choosing a servo<choosing-servo>` section below.

Biggest problem with servos is durability: internal servo gears in cheaper
servos strip easily  when subjected to shock loads. Servos are also very
bad at handling lateral  loads or twisting of the shaft. To avoid having to
